...
function buildUrlsXml(routes: SitemapRoute[]): string {
    const today = new Date().toISOString().slice(0, 10);
...
export function createBlogsSitemap(): void {
    const postRoutes = fetchPostRoutes();
    const routes: SitemapRoute[] = [
        { path: '/posts', changefreq: 'weekly', priority: 1.0 },
        ...postRoutes,
    ];

    const dir = join(process.cwd(), 'public/blogs');
...
export function createProjectsSitemap(): void {
    const projectRoutes = fetchProjectRoutes();
    const routes: SitemapRoute[] = [
