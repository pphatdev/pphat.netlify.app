...
export default async function SignupPage({ searchParams }: SignupPageProps) {
    ...
    if (!session?.user && hasNextAuthSessionCookies(cookieStore)) {
        redirect(buildClearSessionRedirectPath(`/signup?callbackUrl=${encodeURIComponent(params.callbackUrl || '/admin')}`));
    }
    ...
    return (
        <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.16),transparent_35%),linear-gradient(180deg,rgba(15,23,42,0.04),transparent_45%)] px-4 py-10">
