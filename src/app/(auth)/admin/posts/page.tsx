...
export default function AdminPostsPage() {
    ...
    const loadPosts = async () => {
        try {
            const response = await fetch('https://api.pphatdev.workers.dev/v1/api/articles');
            if (response.ok) {
    ...
    return (
        <div className="container max-w-6xl mx-auto p-2 space-y-6 bg-primary/5 border rounded-4xl">
            <div className="flex items-center justify-between px-2 pt-4">
                ...
                <Button asChild className="rounded-full text-foreground bg-background ring ring-foreground/10 hover:bg-primary hover:text-background">
            ...
            <div className='bg-background ring ring-foreground/10 p-4 rounded-3xl'>
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1 relative">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                        <Input placeholder="Search posts..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 border rounded-full shadow-none bg-background" />
                    </div>
                    <div className="flex gap-2  overflow-x-auto">
                        <Button variant={filter === 'all' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('all')} className='rounded-full border border-primary/10'>
                            All ({posts.length})
                        </Button>
                        <Button variant={filter === 'published' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('published')} className='rounded-full border border-primary/10'>
                            Published ({posts.filter(p => p.published).length})
                        </Button>
                        <Button variant={filter === 'draft' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('draft')} className='rounded-full border border-primary/10'>
